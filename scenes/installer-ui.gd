...
func _build_ui() -> void:
	...
	get_window().size = Vector2i(520, 380)
	get_window().min_size = Vector2i(520, 380)
	get_window().max_size = Vector2i(520, 380)
	...
	var folder_icon = load("res://assets/icons/ic_fluent_folder_open_20_regular.svg")
	if folder_icon:
	...
	_apply_button_style(_browse_button)
...
func _on_browse_pressed() -> void:
	# Use native file dialog for folder selection
	var dialog = FileDialog.new()
	dialog.file_mode = FileDialog.FILE_MODE_OPEN_DIR
	dialog.access = FileDialog.ACCESS_FILESYSTEM
	dialog.title = "Choose Install Location"
	dialog.current_dir = _path_edit.text if _path_edit.text != "" else OS.get_environment("LOCALAPPDATA")
	add_child(dialog)
